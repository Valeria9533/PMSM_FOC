...
uint8_t SVPWM(ab_System *p_alphaBeta, ABC_System *p_abc) {
	...
	/*if (p_alphaBeta->b_axis > 0) {
		if (mult_r(p_alphaBeta->b_axis,ONE_DIV_SQRT3) > abs(p_alphaBeta->a_axis)) sector = 2;	
		else sector = (p_alphaBeta->a_axis) > 0 ? 1 : 3;
	}
	else {
		if (mult_r(p_alphaBeta->b_axis,ONE_DIV_SQRT3) < -abs(p_alphaBeta->a_axis)) sector = 5;
		else sector = (p_alphaBeta->a_axis) > 0 ? 6 : 4;
	}

	// Count timings for sectors
	switch (sector) {
		
		int16_t sum_t;
		
		case 1: // sector 1
				t1 = p_alphaBeta->a_axis - mult_r(p_alphaBeta->b_axis,ONE_DIV_SQRT3);
				t2 = 2*mult_r(p_alphaBeta->b_axis,ONE_DIV_SQRT3);
				t0 = INT16_MAX - t1 - t2; 
				...
				break;
		case 2: // sector 2
				t1 = p_alphaBeta->b_axis + mult_r(p_alphaBeta->a_axis,ONE_DIV_SQRT3);
				t2 = p_alphaBeta->b_axis - mult_r(p_alphaBeta->a_axis,ONE_DIV_SQRT3);
				t0 = INT16_MAX - t1 - t2; 
				...
				break;
		case 3: // sector 3
				t1 = 2*mult_r(p_alphaBeta->b_axis,ONE_DIV_SQRT3);
				t2 = - p_alphaBeta->a_axis - mult_r(p_alphaBeta->b_axis,ONE_DIV_SQRT3);
				t0 = INT16_MAX - t1 - t2; 
	...
	// Limitations
	if(switch_time_A < 0) switch_time_A = 0;
	if(switch_time_B < 0) switch_time_B = 0;
	if(switch_time_C < 0) switch_time_C = 0;

	u_Sabc.phaseA = switch_time_A;
	u_Sabc.phaseB = switch_time_B;
	u_Sabc.phaseC = switch_time_C;*/
